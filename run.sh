...
main() {
    ...
    cp /usr/local/etc/intecture/auth_pub.crt agent_bootstrap/auth.crt
    cd agent_bootstrap
...
helper_cert() {
    zmakecert "$1" <<< $'name\nemail@address.com\norg\n'
    mv "$1_secret" "/usr/local/etc/intecture/$1.crt"
    mv "$1" "/usr/local/etc/intecture/$1_pub.crt"
}
...
helper_run() {
    ...
    helper_make INSTALL_DIR=$install_dir
    helper_make INSTALL_DIR=$install_dir test install || return 1
    echo "done"
}
